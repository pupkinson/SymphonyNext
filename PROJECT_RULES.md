...
## Обязательная композиция требований MCP — SN-031-r2
...
SN-031 — неисполняемый агрегат четырнадцати leaf-задач; SN-032 получает готовый MCP command/auth transport от SN-031.W02.02, а SN-042/043/044 по-прежнему ждут полной MCP-приёмки через SN-031.W06. Override SN-032 одновременно заменяет depends_on и interfaces.consumes после проверки исходных значений; семантический граф учитывает обе категории входов. Остальные поля, зависимости и статусы исходного backlog не меняются. При несовпадении baseline hash/expected_depends_on/expected_interfaces_consumes или неподдерживаемом refinement планировщик останавливает импорт, а не теряет новые требования.
